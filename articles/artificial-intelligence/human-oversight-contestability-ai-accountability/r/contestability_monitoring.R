...
appeals <- data.frame(
  ...
  ai_assisted_decision = sample(c(0, 1), n, replace = TRUE,
                                prob = c(0.25, 0.75)),
  appealed = sample(c(0, 1), n, replace = TRUE,
                    prob = c(0.88, 0.12)),
  corrected = 0,
...
success_rate <- aggregate(corrected ~ group, data = subset(appeals, appealed == 1), FUN = mean)
resolution_time <- aggregate(resolution_days ~ group, data = subset(appeals, appealed == 1), FUN = mean)
